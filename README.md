...
If the `run_container.sh` script fails due to an error that says `check if the path is correct and if the daemon is running`, then make sure to open the Docker Desktop app in OSX before running the script. Also make sure to open the XQuartz Desktop app.

If you get an error saying `xhost:  unable to open display ""`, you may need to add `export DISPLAY=:0` to your .bashrc or .zshrc.
## Notes
